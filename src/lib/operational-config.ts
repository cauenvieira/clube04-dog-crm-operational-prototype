...
export interface OperationalConfig {
  unidade: { nome: string; cidade: string; bairro: string; whatsappPrincipal: string; horarioSemana: string; horarioSabado: string; diasAtendimento: string[] };
  origens: string[];
  tentativas: { limiteSemResposta: number; vencidoRecenteDias: number; backlogDias: number; enviarParaLiderancaNoLimite: boolean; permitirProximoContatoSemData: boolean };
  motivos: { perda: string[]; desqualificacao: string[]; lideranca: string[]; nutricao: string[]; prazo: string[] };
  mensagens: { primeiroContato: string; followUp: string; semResposta: string; ultimaTentativa: string; confirmacaoAgendamento: string };
}

export const DEFAULT_CONFIG: OperationalConfig = {
  unidade: { nome: "Clube04 Mogi das Cruzes", cidade: "Mogi das Cruzes", bairro: "Vila Mogilar", whatsappPrincipal: "(11) 99999-9999", horarioSemana: "terça a sexta, 09h às 19h", horarioSabado: "sábado, 08h às 17h", diasAtendimento: ["terça", "quarta", "quinta", "sexta", "sábado"] },
  origens: ["Meta Ads", "WhatsApp", "Instagram orgânico", "Facebook", "Google", "Indicação", "Fachada", "Evento/ação local", "Cadastro manual", "Outro"],
  tentativas: { limiteSemResposta: 12, vencidoRecenteDias: 7, backlogDias: 7, enviarParaLiderancaNoLimite: true, permitirProximoContatoSemData: false },
  motivos: {
...
export function loadConfig(): OperationalConfig {
  ...
  try {
    ...
    cached = raw ? JSON.parse(raw) : DEFAULT_CONFIG;
  } catch { cached = DEFAULT_CONFIG; }
...
export function useOperationalConfig() {
  ...
  const setConfig = useCallback((next: OperationalConfig, actorName?: string, actorRole?: Papel | "sistema") => saveConfig(next, actorName, actorRole), []);
