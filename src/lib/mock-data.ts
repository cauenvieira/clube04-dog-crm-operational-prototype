// Dados mockados do Protótipo Operacional CRM Clube04
...
const TUTORES = [
  "Mariana Souza", "Rafael Oliveira", "Beatriz Costa", "Lucas Pereira", "Camila Almeida",
  "Felipe Santos", "Juliana Lima", "Gustavo Ferreira", "Patrícia Ribeiro", "André Carvalho",
  "Larissa Mendes", "Thiago Barbosa", "Vanessa Rocha", "Bruno Cardoso", "Renata Dias",
  "Diego Martins", "Carolina Nunes", "Eduardo Pinto", "Fernanda Gomes", "Marcelo Araújo",
];
const PETS = ["Thor", "Mel", "Luna", "Bento", "Nina", "Theo", "Bela", "Tobias", "Amora", "Pipoca", "Cacau", "Pingo", "Maya", "Apollo", "Lola", "Zeus", "Frida", "Cookie", "Nala", "Bidu"];
const RACAS = ["SRD", "Shih Tzu", "Maltês", "Yorkshire", "Poodle", "Golden", "Bulldog Francês", "Spitz Alemão", "Pinscher", "Lhasa Apso"];

function randTel(i: number): string {
  const a = 90000 + i * 137;
  const b = 1000 + i * 73;
  return `(11) ${String(a).slice(0,5)}-${String(b).slice(0,4)}`;
}
...
function makeLead(i: number, cfg: Partial<Lead> & { status: StatusOperacional; proximaAcao: ProximaAcao; offsetProx?: number }): Lead {
  ...
  const resultado: ResultadoInteracao =
    ...
    cfg.proximaAcao === "fazer_follow_up" ? "follow_up_agendado" :
    "continuar";
  const lead: Lead = {
    id,
    tutor,
    ...
    atendente: ATENDENTES[i % ATENDENTES.length],
    status: cfg.status,
    proximaAcao: cfg.proximaAcao,
    dataEntrada: daysFromNow(-Math.floor(i % 28) - 1),
    dataProximoContato: cfg.offsetProx !== undefined ? daysFromNow(cfg.offsetProx) : undefined,
    dataUltimoContato: daysFromNow(-Math.floor(i % 5) - 1),
    dataAgendamento: cfg.status === "agendado" ? daysFromNow((cfg.offsetProx ?? 0) + 2) : undefined,
    tentativas: cfg.tentativas ?? 0,
    ultimoResultado: resultado,
    observacao: cfg.observacao ?? (cfg.status === "analise_lideranca" ? "Caso precisa de decisão operacional" : "Registro inicial do protótipo operacional"),
    ...
    historico: [
      {
        id: `${id}-h1`,
        data: daysFromNow(-Math.floor(i % 3) - 1),
        resultado,
        observacao: "Registro automático mockado",
        atendente: ATENDENTES[i % ATENDENTES.length],
        statusNovo: cfg.status,
        proximaAcaoNova: cfg.proximaAcao,
      },
    ],
  };
  return { ...lead, ...cfg };
...
function seedLeads(): Lead[] {
  const configs: Array<Partial<Lead> & { status: StatusOperacional; proximaAcao: ProximaAcao; offsetProx?: number }> = [
    { status: "novo", proximaAcao: "novo_lead", offsetProx: 0, tentativas: 0 },
    { status: "novo", proximaAcao: "novo_lead", offsetProx: 0, tentativas: 0 },
    { status: "em_atendimento", proximaAcao: "retomar_atendimento", offsetProx: 0, tentativas: 1 },
    { status: "em_atendimento", proximaAcao: "retomar_atendimento", offsetProx: -2, tentativas: 3 },
    { status: "em_atendimento", proximaAcao: "retomar_atendimento", tentativas: 2 },
    { status: "follow_up", proximaAcao: "fazer_follow_up", offsetProx: 0, tentativas: 1 },
    { status: "follow_up", proximaAcao: "fazer_follow_up", offsetProx: 2, tentativas: 2 },
    { status: "aguardando_resposta", proximaAcao: "aguardando_resposta", offsetProx: 0, tentativas: 4 },
    { status: "aguardando_resposta", proximaAcao: "aguardando_resposta", offsetProx: -3, tentativas: 6 },
    { status: "aguardando_resposta", proximaAcao: "retomar_atendimento", offsetProx: -12, tentativas: 9 },
    { status: "analise_lideranca", proximaAcao: "revisar_lideranca", offsetProx: 0, tentativas: 12, motivoAnalise: "Tentativas excedidas" },
    { status: "analise_lideranca", proximaAcao: "revisar_lideranca", offsetProx: 0, tentativas: 5, motivoAnalise: "Caso sensível" },
    { status: "agendado", proximaAcao: "registrar_agendamento", offsetProx: 1, tentativas: 1 },
    { status: "convertido", proximaAcao: "sem_proxima_acao", tentativas: 3, observacao: "Cliente fechou pacote mensal" },
    { status: "perdido", proximaAcao: "sem_proxima_acao", tentativas: 5, motivoPerda: "Preço" },
    { status: "desqualificado", proximaAcao: "sem_proxima_acao", tentativas: 1, motivoDesqualificacao: "Precisa de táxi dog" },
    { status: "nutricao", proximaAcao: "sem_proxima_acao", tentativas: 2, motivoNutricao: "Quer receber novidades", campanha: "Lista de transmissão" },
  ];
